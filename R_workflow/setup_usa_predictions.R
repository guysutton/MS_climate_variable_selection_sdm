...
# Mask rasters to keep only the prediction region -> usa
usa_env_layers <- raster::crop(
  selected_predictors, 
  raster::extent(-140, 
                 -60, 
                 10, 
...
usa_env_layers = raster::brick(usa_env_layers)
...
if(!dir.exists(user_dir_2010)){
  ...
  for (i in climPred){
    raster::writeRaster(
      usa_env_layers[[which(names(usa_env_layers) %in% i)]],
...
usa_env_layers <- raster::subset(Env2050_4.5, climPred) 

# Mask rasters to keep only the prediction region
usa_env_layers <- raster::crop( usa_env_layers, 
                                   raster::extent(-140, 
                                                  -60, 
                                                  10, 
                                                  50) )
...
for (layer_name in names(usa_env_layers)) {
  ...
  if (grepl("\\d$", layer_name) && as.numeric(sub(".+_bio_", "", layer_name)) %in% 1:11) {
    ...
    usa_env_layers[[layer_name]] <- usa_env_layers[[layer_name]] / 10
...
# Subset to only keep reduced predictor set
usa_env_layers <- raster::subset(Env2050_8.5, climPred) 
...
# Subset to only keep reduced predictor set
usa_env_layers <- raster::subset(Env2070_4.5, climPred) 
...
# Subset to only keep reduced predictor set
usa_env_layers <- raster::subset(Env2070_8.5, climPred) 
